...
def upload_model():
    ...
    try:
        ...
        try:
            ...
            save_path.unlink(missing_ok=True)  # Delete the bad file
...
def analyze():
    ...
    try:
        ...
        t0 = time.time()
        c1 = run_component1(model, tensor, label)
        c1["elapsed_ms"] = round((time.time() - t0) * 1000)
        results["components"]["component1"] = _to_python(c1)
        log.info(f"[{sid}] C1 done in {c1['elapsed_ms']}ms | δ_fgsm={c1['delta_fgsm']:.4f}")
